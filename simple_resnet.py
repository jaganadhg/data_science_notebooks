#https://towardsdatascience.com/only-numpy-implementing-simple-resnet-for-mnist-classification-with-interactive-code-d58c77064304
...
for iter in range(num_epoch):
    ...
    for current_image_index in range(len(training_images)):
        ...
        # ---- Grad 2 -----
        grad_2_part_common = ((grad_3w1_part_1 * grad_3w1_part_2).dot(w3a.T) + (grad_3_part_common * grad_3H_part_2).dot(w3h.T)) * d_ReLu(H2)
        ...
        # ---- Grad 1 -----
        grad_1_part_common = ((grad_2w1_part_1 * grad_2w1_part_2).dot(w2a.T) + (grad_2_part_common * grad_2H_part_2).dot(w2h.T)) * d_ReLu(H1)
...
for current_batch_index in range(len(testing_images)):
    ...
    print(' Current Predict : ',np.where(H3Soft[0] == H3Soft[0].max())[0], " Ground Truth : ", np.where(current_batch_label == current_batch_label.max())[0] )
